...
func (mh *MsgHandle) SendMsgToTaskQueue(request ziface.IRequest) {
	...
	//log.Printf("Add ConnID=", request.GetConnection().GetConnID()," request msgID=", request.GetMsgID(), "to workerID=", workerID)
	//将请求消息发送给任务队列
...
func (mh *MsgHandle) DoMsgHandler(request ziface.IRequest) {
	handler, ok := mh.Apis[request.GetMessage().GetMsgId()]
	if !ok {
...
func (mh *MsgHandle) AddRouter(msgID uint64, router ziface.IRouter) {
	...
	log.Printf("Add api msgID = ", msgID)
}

//StartOneWorker 启动一个Worker工作流程
func (mh *MsgHandle) StartOneWorker(workerID int, taskQueue chan ziface.IRequest) {
	log.Printf("Worker ID = ", workerID, " is started.")
	//不断的等待队列中的消息
	for {
		select {
